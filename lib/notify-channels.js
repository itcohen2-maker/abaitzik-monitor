...
// Which monitor this is. Every default is Itzik's current address, so a copy
// with no instance.json notifies exactly whom it notified yesterday.
...
const NTFY_TOPIC = inst.ntfy.out;
const MAILBOX = inst.mailbox;
...
*/
const TOKEN_FILE = path.join(inst.dataPath, 'ntfy-token.txt');
function token() {
