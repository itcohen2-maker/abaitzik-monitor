...
(function (factory) {
  ...
  // joins it; a message with re pointing at nothing stands alone, so a typo
  // in re never hides a message.
  function splitThreads(chat) {
    var list = (chat || []).slice().sort(byAt);
    var ids = {};
    list.forEach(function (m) { ids[keyOf(m)] = true; });
    var roots = {};
    var order = [];
    list.forEach(function (m) {
      if (m.re && ids[m.re]) return;
      var k = keyOf(m);
