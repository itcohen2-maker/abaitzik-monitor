...
/*
  The form sends "שתן=280". He writes "שתן 280".

  18.9: the column was asked for in a voice note, in the words "make a column
  for urine 280, add it to the table". A reader that only understands the
  equals sign understands the form and not the man, and the whole point of the
  line is that he can also just say it.
*/
const URINE = /שתן\s*[=:]?\s*(-?\d+(?:\.\d+)?)/;
...
function parse(text) {
  ...
  const line = body.split(/\r?\n/).find(l => WORDS.test(l) && (/\d\s*=/.test(l) || URINE.test(l)));
  if (!line) return null;
  ...
  /*
    Urine goes on the same row.

    Itzik, 18.9: "make a column for urine, 280, add it to the table." It is not
    a drain, but it is measured in the same jug at the same hour and it is the
    other half of the same question a surgeon asks after an abdominal
    operation: what went in, what came out, and by which route. Keeping it on a
    second screen would mean reading two screens to answer one question.
  */
  const u = line.match(URINE);
  const urine = u ? Number(u[1]) : null;
  if (!found && urine === null) return null;
  const dot = line.indexOf(' · ');
  /*
    The urine reading has its own column, so it does not also belong in the
    note. He writes it inside the free text ("... 4=45 · שתן 350") and the
    first version copied that text straight into the note, which put the same
    number on the row twice. What is left after taking the number out is what
    he actually wanted to say, and if nothing is left there is no note.
  */
  let note = dot === -1 ? '' : line.slice(dot + 3).trim();
  if (u) note = note.replace(u[0], '').replace(/\s*·\s*/g, ' · ').replace(/^[\s·,]+|[\s·,]+$/g, '').trim();
  return { vals: vals, urine: urine, note: note };
}
...
function record(dir, text, at) {
  ...
  /*
    Same round, not same day.

    The first version folded anything sent on the same date into one row, which
    is right for the four messages he sends twenty four seconds apart while he
    empties one tube after another. It is wrong for the next round hours later:
    on 18.9 the 10:04 numbers landed on top of the 08:41 ones and the row said
    "drain 1 corrected from 65 to 25". Nothing was corrected. He measured twice,
    and a log that turns the morning reading into a typo has lost the thing it
    exists to show, which is the direction.

    So a send joins the row it is actually part of: the newest row within half
    an hour of it. Anything later starts its own.
  */
  ...
  const existing = fs.readdirSync(dir)
    ...
    .filter(function (f) {
      try {
        ...
        return Math.abs(when.getTime() - Date.parse(r.at)) <= ROUND_MS;
  ...
  if (parsed.urine !== null && parsed.urine !== undefined) {
    if (out.row.urine !== parsed.urine) { out.row.urine = parsed.urine; out.changed = true; }
  }
