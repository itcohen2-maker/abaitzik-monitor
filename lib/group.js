...
/*
  His personal code off the end of anything that is going to be looked at.

  The settings screen promises the code is never shown, and it is not shown
  there. But every send carries it as the last line of the body, and a plain
  message was stored and drawn exactly as it arrived, so the code sat at the
  bottom of his own bubble and came back on every refresh. He asked on 19.9
  whether that is what he was seeing. It was. Stripped on the way in, so it is
  not written to disk, and again on the way to the page, so the messages
  already on disk stop showing it too.
*/
function stripCode(text) {
  return String(text == null ? '' : text)
    .replace(/\s*קוד\s*[0-9]{3,6}\s*$/, '')
    // And anywhere else it turns up. Sessions quoted it back at him as proof of
    // who approved a block, so it sat in the blocklist reasons too, in the
    // middle of a sentence where the tail cut above never reached it.
    .replace(/קוד\s*[0-9]{3,6}/g, 'קוד ••••')
    .trim();
}
...
module.exports = { groupOf, isCaption, groupText, isAudio, audioIn, captionFrom, stripCode };
